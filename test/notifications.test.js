...
describe('sendNotification', () => {
  ...
  it('logs error on darwin if osascript fails', async () => {
    Object.defineProperty(process, 'platform', { value: 'darwin' });
    child_process.execFile.mockImplementation((cmd, args, cb) => cb(new Error('osascript error')));

    await sendNotification('Test', 'Message');

    expect(child_process.execFile).toHaveBeenCalled();
    expect(logger.error).toHaveBeenCalledWith('Failed to send macOS notification: osascript error');
  });
